...
AEF.tree.mortdat<-AEF.rem.tree.dat%>%
  left_join(AEF.coords, by=c("STANDID" = "LOCATION_CODE"))%>%
  filter(TREE_STATUS == "6")%>% #remove dead/missing trees
  mutate(site = STANDID, 
  ...
  group_by(std_id, data, ecosystem, site, plot, transect, latitude, longitude, 
         year, month, day, spp_full_name, genus, species, tree_id, stem_id)%>%
  slice_min(year)%>%
  summarise(mort_year = year)%>%
  mutate(notes = "plot marks 'PLOTNUMBER', stem_id marks 'TAG'")


AEF.tree.mortdat.fin<-as.data.frame(AEF.tree.mortdat)


duplicates<- AEF.tree.mortdat %>%
...
mort.check <- ForestGeo.SERC.dat %>%
  group_by(Quadrat, TreeID) %>%
  summarize(alive = any(Status == "alive"), dead = any(Status == "dead")) %>%
  filter(alive & dead)

dbh.mort<-AEF.tree.DIAdat%>%
  left_join(AEF.tree.mortdat, by=c("std_id", "site", "plot", "transect", "year", "month", "day", "spp_full_name", "genus", "species", "tree_id", "stem_id"))

dbh.mort<-AEF.tree.DIAdat%>%
  left_join(AEF.tree.mortdat.fin%>%
              select(std_id, site, plot, transect, 
                     spp_full_name, genus, species,
                     tree_id, stem_id, 
                     mort_year), 
            by=c("std_id", "site", "plot", "transect", 
                 "spp_full_name", "genus", "species", 
                 "tree_id", "stem_id"))

AEF.rem.tree.dat%>%filter(TREEID == "CH11000100001")%>%select(YEAR, TREE_STATUS)
AEF.rem.tree.dat%>%filter(TREEID == "CH11000100003")%>%select(YEAR, TREE_STATUS)
...
#Save Files
write.csv(AEF.tree.mortdat.fin, file="Mortality.Species.Indv.Forest.AEF.LTER.csv", row.names = F)
############################################################