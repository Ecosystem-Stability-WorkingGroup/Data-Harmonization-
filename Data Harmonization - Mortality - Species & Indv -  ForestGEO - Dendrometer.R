...
#ForestGeo - Dendrometer - North America - SCBI
# - CSV Name
ForestGeo.SCBI.den.mortdat<-ForestGeo.SCBI.den.dat%>%
  filter(Genus != "")%>%
  mutate(site = Site, 
         ...
         latitude = Latitude,
         longitude =  Longitude)%>%
  filter(!is.na(DendroMeasure) & DendroMeasure >0)%>%    
...
#Save Files
write.csv(ForestGeo.SCBI.den.mortdat, file="Mortality.Species.Indv.ForestGEO.SCBI.Dendro.csv", row.names = F)
############################################################
