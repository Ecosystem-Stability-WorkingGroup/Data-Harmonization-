...
#Growth Columns
#FIA.growth<-read_csv("C:/Users/rfidler/Desktop/Powell Ecosystem Stability/Data/Forests/FIA/ENTIRE_TREE_GRM_COMPONENT.csv",
#                     col_select = c("PLT_CN", "STATECD", "TRE_CN", "GROWCFAL_FOREST"))
...
#############################################################
#Clearing Space and Loading Remaining Data
rm(FIA.veg)


library(data.table)
FIA.growth <- fread("C:/Users/rfidler/Desktop/Powell Ecosystem Stability/Data/Forests/FIA/ENTIRE_TREE_GRM_COMPONENT.csv",
                    select = c("PLT_CN", "STATECD", "TRE_CN", "GROWCFAL_FOREST"))
detach("package:ldata.table", unload = TRUE)

FIA.growth$PLT_CN<-as.double(FIA.growth$PLT_CN)
FIA.growth$TRE_CN<-as.double(FIA.growth$TRE_CN)
############################################################
#Formatting Data
FIA.tree.nppdat<-FIA.tree.dat%>%
  filter(STATUSCD == 1)%>%       #Live Trees Only
  left_join(FIA.coords.trim, by = c("PLT_CN" = "CN", 
                                    "STATECD" = "STATECD", 
                                    "UNITCD"="UNITCD", 
                                    ...
                                    "INVYR" = "INVYR",
                                    "PLOT" = "PLOT"))%>%
  left_join(FIA.growth, by = c("PLT_CN" = "PLT_CN",
                               "STATECD" = "STATECD",
                               "CN" = "TRE_CN"))%>%
  mutate(year = MEASYEAR,
         ...
         npp = GROWCFAL_FOREST,
         npp_units = "cubic ft yr-1")%>%
  filter(!is.na(GROWCFAL_FOREST))%>%
...
FIA.tree.nppdat.fin<-FIA.tree.nppdat%>%
  filter(latitude !="NaN")

duplicates<- FIA.tree.nppdat.fin %>%
...
#Save Files
write.csv(FIA.tree.nppdat.fin, file="Diameter.Species.Indv.Forest.FIA.NPP.csv", row.names = F)
############################################################
