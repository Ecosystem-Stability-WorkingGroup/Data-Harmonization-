...
#Florida Coastal Everglades (FCE) LTER - Florida Bay Productivity
# - LT_PP_Fourqurean_004.csv
FCE.seagrass.npp.nppdat<-FCE.seagrass.npp.dat%>%
...
str(FCE.seagrass.npp.dat)
str(FCE.seagrass.npp.nppdat)

#Species checking
spp.check<-FCE.seagrass.npp.nppdat%>%
  select(spp_full_name, genus, species)%>%
  distinct()

#Backcheck for aggregation levels
check1<-FCE.seagrass.npp.nppdat%>%
...
#Save Files
write.csv(FCE.seagrass.npp.nppdat, file="NPP.Species.Indv.Seagrass.FCE.LTER.NPP.NPP.csv", row.names = F)
############################################################
