...
############################################################
#Creating Unified Percent Cover Column

huh<-NERR.MV.dat2%>%filter(Cover =="R")
unique(huh$Reserve)

huh2<-NERR.MV.dat2%>%filter(Cover != "R" & Cover != "% Cover")%>%mutate(cov2=as.numeric(Cover))
min(huh2$Cover, na.rm=T)

huh3<-NERR.MV.dat2%>%filter(!is.na(Cover) & !is.na(Percent.Cover))

NERR.MV.dat2<-NERR.MV.dat2%>%
  mutate(percent_cover = ifelse(is.na(Cover), Percent.Cover, Cover))%>%
  mutate(percent_cover = ifelse(percent_cover=="R", 0.1, percent_cover))%>%
  mutate(percent_cover = case_when(percent_cover == "for each species" ~ NA,
                                   percent_cover == "% Cover" ~ NA,
                                   percent_cover == "whole number, R" ~ NA,
                                   percent_cover == "ND" ~ NA,
                                   TRUE ~ percent_cover))%>%
  mutate(percent_cover = as.numeric(percent_cover))

unique(NERR.MV.dat2$percent_cover)
...
NERR.MV.pcdat<-NERR.MV.dat3%>%
  ...
  filter(!is.na(latitude))%>%             #Filtering out sites with no GPS
  filter(!is.na(percent_cover) & percent_cover >0)%>%   #Filtering out points without percent cover measurements
  distinct()%>%
  mutate(notes = "plot marks 'Plot/Subplot', transect marks 'Transect/Rep', aggregation is at plot/subplot/transect/y/m/d, many sites with >100% cover")%>%
...
spp.check<-NERR.MV.pcdat%>%
  select(spp_full_name, genus, species)%>%
  distinct()


check1<-NERR.MV.pcdat%>%
...
#Save Files
write.csv(NERR.MV.pcdat, file="Percent.Cover.Species.SMEW.NOAA.NERR.MV.csv", row.names = F)
############################################################
